...
class DGNObjectSet(ObjectSet):
    ...
    def usd(self, key: str):
        cfg = self.cfg
        return F'{cfg.usd_path}/{key}/{key}.usd'
    ...
    def pose(self, key: str):
        ...
        if (((key not in self.poses) or (self.poses[key] is None))
            ...
            pose = sample_stable_poses(self.hulls[key],
                                       # FIXME(ycho): hardcoded table height
                                       0.4,
                                       16)
            self.poses[key] = pose
...
def _convert_usd(urdf: str, out_path: str):
    out_path = Path(out_path)
    key = Path(urdf).stem
    usd_path = ensure_directory(out_path / 'usd' / key)

    urdf_converter_cfg = UrdfConverterCfg(
        asset_path=urdf,
        usd_dir=usd_path,
        usd_file_name=f'{key}.usd',
        fix_base=False,
...
def _convert_urdf_to_usd():
    ...
    # names = names[:100]
    # no real "label" available for now...
    names = [dgn.urdf(key) for key in dgn.keys()]
    # urdf (OK?)
