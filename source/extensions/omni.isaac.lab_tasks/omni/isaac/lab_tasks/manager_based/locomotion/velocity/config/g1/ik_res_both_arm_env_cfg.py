...
def action_norm_l2(env,
    ...
    if "Residual_action_norm" not in env.reward_manager.episode_stat_sums.keys():
        env.reward_manager.episode_stat_sums["Raw_Residual_action_norm"] = \
    ...
    env.reward_manager.episode_stat_sums["Raw_Residual_action_norm"] += \
        th.norm(action.raw_actions, dim=-1)
    

    # return th.sum(th.square(action.processed_actions), dim=-1)
    return th.sum(th.square(action.raw_actions), dim=-1)

def action_limits(env, 
                  action_name:str,
                  limits:List[float] = [-0.5, 0.5]
                  ):
    action = env.action_manager.get_term(action_name)
    # compute out of limits constraints
    out_of_limits = -(
        action.raw_actions - limits[0]
    ).clip(max=0.0)
    out_of_limits += (
        action.raw_actions - limits[1]
    ).clip(min=0.0)
...
class G1Rewards:
    ...
    residual_action_limit_right = RewTerm(
        func=action_limits,
        weight=-0.1,
        params={
            "action_name": 'right_arm_res'
